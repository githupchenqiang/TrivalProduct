...
@interface ThemeDetail ()<UIWebViewDelegate>
...
- (void)viewDidLoad {
    ...
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:self.themeItem.jump_url]];
...
- (void)buttonItemAction{
    [self shareControlWithUrl:self.themeItem.jump_url];
}
...
- (void)shareTextToPlatformType:(UMSocialPlatformType)platformType ShareUrl:(NSString *)url
{
    ...
    //创建网页内容对象
    NSURL *imageUrl = [NSURL URLWithString:self.themeItem.img_url];
    UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:imageUrl]];
    UMShareWebpageObject *shareObject = [UMShareWebpageObject shareObjectWithTitle:self.themeItem.title descr:self.themeItem.sub_title thumImage:image];
    //设置网页地址
    shareObject.webpageUrl = self.themeItem.jump_url;
    //分享消息对象设置分享内容对象
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    [webView stringByEvaluatingJavaScriptFromString:@"document.documentElement.getElementsByClassName('fixe-bar')[0].style.display = 'none'"];
    [webView stringByEvaluatingJavaScriptFromString:@"document.getElementsByClassName('sub-box')[0].style.display = 'none'"];
    [webView stringByEvaluatingJavaScriptFromString:@"document.getElementsByClassName('hd')[0].style.display = 'none'"];
    [webView stringByEvaluatingJavaScriptFromString:@"document.getElementsByClassName('bd')[0].style.display = 'none'"];
}
...
//    //假设只是分析 "ABCD://"
//    
//    NSRange range = [strXXX rangeOfString:self.themeItem.jump_url];
