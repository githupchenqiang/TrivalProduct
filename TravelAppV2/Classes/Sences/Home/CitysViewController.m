...
-(void)getCityDic{
    ...
    for (NSInteger i=1; i<_cityArray.count; i++) {
        CitySModel *currentModel=_cityArray[i];
        ...
        CitySModel *preModel = _cityArray[i-1];
...
- (UITableView *)cityTableView{
    if (!_cityTableView) {
        _cityTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,[PublicSource NavigationBarHeight] + 44, self.view.width, self.view.height-64.0f-44.0f) style:UITableViewStylePlain];
...
- (UISearchBar *)searchBar{
    if (!_searchBar) {
        _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, [PublicSource NavigationBarHeight], self.view.width, 44.0f)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!isSearch) {
        ...
        CitySModel *model = self.cityArray[index];
